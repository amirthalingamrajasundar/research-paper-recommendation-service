...
VECTORIZER_PATH = os.getenv("VECTORIZER_PATH", str(PROJECT_ROOT / "models/tfidf/vectorizer.joblib"))
MATRIX_PATH = os.getenv("MATRIX_PATH", str(PROJECT_ROOT / "models/tfidf/embeddings.joblib"))
MODEL_NAME = "tfidf"


class TFIDFService:
    """TF-IDF based recommendation service."""
    ...
    def load(self):
        ...
        print(f"Loading TF-IDF model from {VECTORIZER_PATH}...")
        self.vectorizer = load(VECTORIZER_PATH)
        self.tfidf_matrix = load(MATRIX_PATH)
    ...
    def search(self, query: str, page: int = 1, limit: int = 6) -> tuple:
        ...
        non_zero_mask = similarities[top_indices] > 0
...
# FastAPI app
app = FastAPI(
    title="TF-IDF Recommendation Service",
    version="1.0.0",
    lifespan=lifespan
)
...
async def search(
    ...
    model: str = Query(..., description="Model name (ignored, always uses tfidf)"),
    ...
    papers = [
        row_to_paper(row, similarity_score=score)
